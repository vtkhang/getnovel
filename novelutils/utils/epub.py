...
class EpubMaker:
    ...
    def from_url(self, url: str, duplicate_chapter: bool, start: int, stop: int) -> None:
        ...
        c.convert_to_xhtml(duplicate_chapter=duplicate_chapter, rm_result=True, lang_code=p.get_langcode())
    ...
    def from_raw(self, raw_dir_path: PathStr, duplicate_chapter: bool, lang_code: str) -> None:
        ...
        c.convert_to_xhtml(duplicate_chapter=duplicate_chapter, rm_result=True, lang_code=lang_code)
    ...
    def _copy_to_epub(self, xhtml_dir: Path) -> None:
        ...
        copy_tree(str(Path(__file__).parent / 'storage' / 'template'), str(self.tmp_edp))
    ...
    def _make_epub(self, xhtml_files: ListPath, lang_code: str):
        ...
        for item in xhtml_files[2:]:
            ...
            opf_itemref_tag_list.append(
                itemref.format(
                    chapter_name=chapter_name
                )
            )
            nav_li_tag_list.append(nav_li.format(
                chapter_name=chapter_name,
                chapter_title=chapter_title
            ))
            index = index + 1
