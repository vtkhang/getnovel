...
class NovelCrawler:
    ...
    def crawl(self, rm_raw: bool, start_chap: int, stop_chap: int, clean: bool = True) -> None:
        ...
        if stop_chap < start_chap and stop_chap != -1:
            raise CrawlNovelError('Index of stop chapter need to be greater than start chapter or equal -1')
        if rm_raw is True:
    ...
    def _get_spider_name(self) -> str:
        ...
        spider_name = ''
        spider_name_list = (
            'bachngocsach', 'sstruyen', 'tangthuvien', 'truyencv',
            'truyendkm', 'truyenfull', 'webtruyen', 'metruyenchu', 'vtruyen', 'nuhiep', 'truyenchu',
            'ptwxz', 'uukanshu', '69shu', 'twpiaotian'
        )
...
def validate_url(url: str):
    ...
    if len(url) > 2048:
        raise CrawlNovelError(f'URL exceeds its maximum length of 2048 characters (given length={len(url)})')
