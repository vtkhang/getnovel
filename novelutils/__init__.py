...
def crawl_func(args):
    ...
    p.crawl(rm_raw=not args.keep_raw, start_chap=args.start, stop_chap=args.stop, clean=args.clean)


def convert_func(args):
    """Convert process."""
    c = FileConverter(args.raw_dir, args.result_dir)
    c.convert_to_xhtml(lang_code=args.lang_code, duplicate_chapter=args.dup_chap, rm_result=not args.keep_result)
...
def _build_parser():
    ...
    rm_dup.add_argument('--result_dir', type=str,
                         default=None, metavar='RESULT_PATH',
                         help='path to result directory (default: replace files in raw directory)')
    rm_dup.add_argument('raw_dir', type=str,
                         help='path to raw directory')
    rm_dup.set_defaults(func=rm_dup_func)
    # epub parser
    epub = subparsers.add_parser('epub', help='make epub')
    subparsers_epub = epub.add_subparsers(
        title='modes',
        help='supported modes'
    )
    # epub from_url parser
    from_url = subparsers_epub.add_parser('from_url', help='make epub from web site')
    from_url.add_argument('--dup_chap',
                          action='store_true',
                          help='if specified, remove duplicate chapter title (default:  %(default)s)')
    from_url.add_argument('--start', type=int,
                          default=1,
                          help='start chapter index (default:  %(default)s)')
    from_url.add_argument('--stop', type=int,
                          default=-1,
                          help='stop chapter index, input -1 to get all chapters (default:  %(default)s)')
    from_url.add_argument('url', type=str,
                          help='full web site to novel info page')
    from_url.set_defaults(func=epub_from_url_func)
    # epub from_raw parser
    from_raw = subparsers_epub.add_parser('from_raw', help='make epub from raw directory')
    from_raw.add_argument('--dup_chap',
                          action='store_true',
                          help='if specified, remove duplicate chapter title (default:  %(default)s)')
    from_raw.add_argument('--lang_code',
                          default='vi',
                          help='language code of the novel (default: %(default)s)')
    from_raw.add_argument('raw_dir', type=str,
                          help='path to raw directory')
