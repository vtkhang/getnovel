...
class TWpiaotianSpider(scrapy.Spider):
    ...
    def parse(self, response: scrapy.http.Response, **kwargs):
        ...
        menu_link = 'https://www.twpiaotians.com/piaotianlist/{0}/dir.html'.format(t.rsplit('/', 1)[1].split('.')[0])
        yield scrapy.Request(
...
def get_info(response: scrapy.http.Response, save_path: Path):
    ...
    types = response.xpath('//div[@class="bookintro"]/p[@class="label"]/span[2]/text()').getall()
    foreword = response.xpath('//div[@class="bookintro"]/p[@class="intro"]/text()').getall()
    info = list()
    info.append(title)
    info.append(author)
...
def get_content(response: scrapy.http.Response, save_path: Path):
    ...
    # get content
    content: list = response.xpath('//div[contains(@class,"pt-read-text")]/p/text()').getall()
