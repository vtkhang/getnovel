...
def get_info(response: scrapy.http.Response, save_path: Path):
    ...
    foreword = response.xpath(
        "/html/body/div[5]/div[4]/div[1]/div[1]/div[1]/p/text()"
    ).getall()
    info = list()
    info.append(title)
    info.append(author)
...
def get_content(response: scrapy.http.Response, save_path: Path):
    ...
    # get chapter
    chapter = (
        response.xpath('//div[contains(@class,"chapter")]/h2/text()')
        .get()
        .replace("\xa0", " ")
    )
    # get content
    content = response.xpath(
        '//div[contains(@class,"box-chap")]//text()[not(parent::a)]'
    ).getall()
