...
class TruyenCvSubSpider(scrapy.Spider):
    ...
    def parse(self, response: scrapy.http.Response, **kwargs):
        ...
        yield scrapy.Request(
            response.xpath('//div[@class="nh-thumb nh-thumb--210 shadow"]/img/@src').get(),
        ...
        total_chapter_str = response.xpath(
            '//*[@id="nav-tab-chap"]/span[2]/text()').get()
        ...
        try:
            ...
            raise scrapy.exceptions.CloseSpider(
                reason='Can\'t convert to integer.') from e
        self.base_url = response.request.url
        self.total_chapter = total_chapter
        yield scrapy.Request(url=self.base_url + r'/chuong-' + str(self.start_chap),
    ...
    def parse_content(self, response: scrapy.http.Response):
        ...
        if response.meta['id'] >= self.total_chapter or response.meta['id'] == self.stop_chap:
            raise scrapy.exceptions.CloseSpider(reason='Done')

        yield scrapy.Request(url=self.base_url + r'/chuong-' + str(response.meta['id'] + 1),
...
def get_info(response: scrapy.http.Response, save_path: Path):
    ...
    title = response.xpath(
        '//h1[@class="h3 mr-2"]/a/text()').get()  # extract title
    author = response.xpath(
        '//ul[@class="list-unstyled mb-4"]/li[1]/text()').get()  # extract author
    types = response.xpath(
        '//ul[@class="list-unstyled mb-4"]/li/a/text()').getall()  # extract types
    foreword = response.xpath(
        '//div[@class="content"]/p/text()').getall()  # extract description
    info = list()
    info.append(title)
    info.append(author)
