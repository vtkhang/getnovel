...
class BachNgocSachSpider(scrapy.Spider):
    ...
    def parse(self, response: scrapy.http.Response, **kwargs):
        ...
        yield scrapy.Request(url=response.url + r'/muc-luc?page=all',
                             callback=self.parse_start_chapter)
    ...
    def parse_start_chapter(self, response: scrapy.http.Response):
        ...
        yield scrapy.Request(
            url='https://bachngocsach.com' +
                response.xpath('//div[@class="mucluc-chuong"]/a/@href').getall()[self.start_chap - 1],
    ...
    def parse_content(self, response: scrapy.http.Response):
        ...
        yield scrapy.Request(url='https://bachngocsach.com' + t[0],
...
def get_info(response: scrapy.http.Response, save_path: Path):
    ...
    foreword = response.xpath('//div[@id="gioithieu"]/p/text()').getall()
    info = list()
    info.append(title)
    info.append(author)
